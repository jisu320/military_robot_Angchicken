...
class RealsenseNode(Node):
    def __init__(self):
        ...
        self.clip_dis_maxm = 0.05  # 최대 길이 범위 조정 (m)
        self.clip_dis_minm = 0.05  # 최소 ..
    ...
    def AirdropDetect(self):
        if self.state == 'airdrop_detection':
                ...
                if confidence >= CONFIDENCE_THRESHOLD and self.class_list[label] == 'box':
                    if confidence >= 0.7:
                        ...
                        return_value = cal_angle(accel_array[1], accel_array[2], detect_point[0], detect_point[1],
                                                 circle_dist)
                        real_x, real_z, yawing_angle, imu_angle = float(return_value[0] * 100), float(return_value[1])*100, float(
                            return_value[2] * 57.2958), float(return_value[3] * 57.2958)
                        print("real_dist: %f [cm]" % real_x)
                        print("real_z: %f [cm]" % real_z)
            ...
            if airdrop_detected is not None and real_x !=0 and yawing_angle != 0:
                ...
                self.state = 'commuincation_odrive'
    ...
    def TrackDepth(self):
        ...
        if self.state == 'commuincation_arduino': #commuincation with arduino
            ...
            if self.number_msg == None:
                self.get_logger().info('Publish to gripper move down')
                self.publish_number_to_arduino(2) # 그리퍼 내리고 잡아라는 명령

            if self.received_arduino_number == 2: #need to change True

                if self.number_msg != 3:
                    self.get_logger().info('publish to gripper up')
                    self.publish_number_to_arduino(3)
                    self.number_msg = 3
                     # 그리퍼를 들으라는 명령
                else :
                    print(self.received_arduino_number)
                    self.get_logger().info('Wait for lift gripper')
                    time.sleep(0.1)

            elif self.received_arduino_number == 3:
                self.get_logger().info('go running to depth_tracking')
                self.state = 'depth_tracking'
                time.sleep(0.1)

            else:
                print(self.received_arduino_number)
                self.get_logger().info('Wait for Arduino_subscribe')
        ...
        if self.state == 'depth_tracking':
            ...
            depth_roi = depth_i[self.roi_start_row:self.roi_end_row, self.roi_start_col:self.roi_end_col]
            center_x = depth_roi.shape[1] // 2
            center_y = depth_roi.shape[0] // 2
            depth_at_center = depth_roi[center_y, center_x]
            bg_removed = np.where((depth_i_3d > depth_at_center + self.clip_dis_max) | (depth_i_3d < depth_at_center - self.clip_dis_min) | (depth_i_3d <= 0), grey_color, masked_color_i)

            #binary_mask = np.where((depth_i_3d > depth_at_center + self.clip_dis_max) | (depth_i_3d < depth_at_center - self.clip_dis_min) | (depth_i_3d <= 0), 0, 1)
            l_mask = bg_removed[self.roi_start_row:self.roi_end_row, self.roi_start_col:self.cen_x]
            r_mask = bg_removed[self.roi_start_row:self.roi_end_row, self.cen_x:self.roi_end_col]
