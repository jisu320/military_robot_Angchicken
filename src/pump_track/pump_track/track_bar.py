...
class CameraStateSubscriber(Node):
    def __init__(self):
        ...
        # add        
        self.lower_bound = np.array((40, 10, 10))
        self.upper_bound = np.array((130, 255, 255))

        cv2.namedWindow('HSV Trackbars')

        cv2.createTrackbar('H L', 'HSV Trackbars', self.lower_bound[0], 180, self.update_hue_lower)
        cv2.createTrackbar('H U', 'HSV Trackbars', self.upper_bound[0], 180, self.update_hue_upper)
        cv2.createTrackbar('S L', 'HSV Trackbars', self.lower_bound[1], 255, self.update_saturation_lower)
        cv2.createTrackbar('V L', 'HSV Trackbars', self.lower_bound[2], 255, self.update_value_lower)
        cv2.createTrackbar('S U', 'HSV Trackbars', self.upper_bound[1], 255, self.update_saturation_upper)
        cv2.createTrackbar('V U', 'HSV Trackbars', self.upper_bound[2], 255, self.update_value_upper)
    ...
    def update_value_lower(self, value):
    ...
    def update_value_upper(self, value):
