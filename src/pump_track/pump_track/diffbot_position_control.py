...
class ODriveControlNode(Node):
    def __init__(self):
        ...
        self.motor_position_publisher = self.create_publisher(
            Bool,
            '/motor/posision',  # Update with your desired yawing_angle topic
            10
        )
    ...
    def position_control_drive(self, position_setpoint_l, position_setpoint_r):
        ...
        while self.my_drive.axis0.encoder.pos_estimate <= position_setpoint_l or self.my_drive.axis1.encoder.pos_estimate >= -position_setpoint_r:
            time.sleep(0.1)
            self.get_logger().info('Current Position setpoint is axis0: %.2f, axis1: %.2f' %
                               (self.my_drive.axis0.encoder.pos_estimate,
                                self.my_drive.axis1.encoder.pos_estimate))
            
        self.get_logger().info('position moved end.')
        motor_position=True
        motor_position_msg = Bool()
        motor_position_msg.data = motor_position
        self.motor_position_publisher.publish(motor_position_msg)
...
def calculate_position_setpoint_l(real_x, yawing_angle):
    ...
    if yawing_angle > 0:
        ...
        position_setpoint_l += 2 *(10/90) * abs(yawing_angle)
    elif yawing_angle < 0:
        # Turn left by the absolute value of yawing_angle
        position_setpoint_l += 0

    # Move forward by the specified real_x value
    position_setpoint_l += 0.385 * (real_x-19)  #here!!!!!!!!!!!!!!!!!!!
...
def calculate_position_setpoint_r(real_x, yawing_angle):
    ...
    if yawing_angle > 0:
        # Turn right by the yawing_angle value
        position_setpoint_r += 0
    elif yawing_angle < 0:
        # Turn left by the absolute value of yawing_angle
        position_setpoint_r += 2 * (10/90) * abs(yawing_angle)

    # Move forward by the specified real_x value
    position_setpoint_r += 0.385 * (real_x-19) #here!!!!!!!!!!!!!
